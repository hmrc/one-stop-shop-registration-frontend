...
class StartDateSpec extends AnyFreeSpec with Matchers with ScalaCheckPropertyChecks with OptionValues {
    ...
    "must serialise" in {
      ...
      forAll(gen) {
        startDate =>
          ...
          Json.toJson(startDate) mustEqual JsString(startDate.toString)
      }
    }
  }
}
