...
class PeriodServiceSpec
  ...
  ".getCurrentReturnPeriod" - {
    ...
    "if today is 11th Jan then current period is Jan - March 2022" in {

      val instant = Instant.parse("2022-01-11T12:00:00Z")
      val stubClock: Clock = Clock.fixed(instant, ZoneId.systemDefault)
      val service = new PeriodService(stubClock)

      val commencementDate = LocalDate.of(2022, 1, 11)

      val expectedPeriod = Period(2022, Q1)
      service.getFirstReturnPeriod(commencementDate) mustBe expectedPeriod
    }

    "if today is 1st Jan then current period is Jan - March 2022" in {
    ...
    "if today is 30th June then current period is April - July 2022" in {

      val instant = Instant.parse("2022-06-30T12:00:00Z")
      val stubClock: Clock = Clock.fixed(instant, ZoneId.systemDefault)
      val service = new PeriodService(stubClock)

      val commencementDate = LocalDate.of(2022, 6, 30)

      val expectedPeriod = Period(2022, Q2)
      service.getFirstReturnPeriod(commencementDate) mustBe expectedPeriod
    }

    "if today is 12th February and my commencement date is 01st April then current period is April - July 2022" in {

      val instant = Instant.parse("2022-02-12T12:00:00Z")
      val stubClock: Clock = Clock.fixed(instant, ZoneId.systemDefault)
      val service = new PeriodService(stubClock)

      val commencementDate = LocalDate.of(2022, 4, 1)
