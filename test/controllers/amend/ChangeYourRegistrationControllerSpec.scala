...
class ChangeYourRegistrationControllerSpec extends SpecBase with MockitoSugar with SummaryListFluency with BeforeAndAfterEach {
  ...
  private val request = AuthenticatedDataRequest(FakeRequest("GET", "/"), testCredentials, vrn, Some(registration), emptyUserAnswers, None, 0, None, None, None)
      ...
      "when the user has answered all necessary data and submission of the registration succeeds" - {
        ...
        "must audit the event and redirect to the next page when enrolment is not enabled" in {
          ...
          running(application) {
            ...
            val result = route(application, request).value
            val dataRequest = AuthenticatedDataRequest(request, testCredentials, vrn, None, userAnswers, None, 0, None, None, None)
        ...
        "the user is redirected when the incomplete prompt is shown" - {
          ...
          "to Trading Name when trading names are not populated correctly" in {
            ...
            running(application) {
              ...
              status(result) `mustBe` SEE_OTHER
              redirectLocation(result).value `mustBe` controllers.routes.TradingNameController.onPageLoad(AmendMode, Index(0)).url
            }
          }
      ...
      "when the submission fails because of a technical issue" - {
        ...
        "the user is redirected to the Error Submitting Registration page and their answers are saved" in {
          ...
          running(application) {
            val request = FakeRequest(POST, amendRoutes.ChangeYourRegistrationController.onSubmit(false).url)
            val result = route(application, request).value
            val dataRequest = AuthenticatedDataRequest(request, testCredentials, vrn, None, basicUserAnswersWithVatInfo, None, 0, None, None, None)
