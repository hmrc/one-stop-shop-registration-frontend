...
class AuthActionSpec extends SpecBase with MockitoSugar with BeforeAndAfterEach {
  ...
  private val vatEnrolmentWithRegistration =
    Enrolments(
      Set(
        Enrolment(
          "HMRC-MTD-VAT",
  ...
  "Auth Action" - {
    ...
    "when registration enrolments enabled" - {
        ...
        "must succeed" in {
          ...
          running(application) {
            ...
            when(mockAuthConnector.authorise[RetrievalsType](any(), any())(any(), any()))
              .thenReturn(Future.successful(Some(testCredentials) ~ vatEnrolmentWithRegistration ~ Some(Organisation) ~ ConfidenceLevel.L50 ~ Some(User)))
      ...
      "when the user is logged in as an Individual with a VAT enrolment, strong credentials and confidence level 200" - {
        ...
        "must succeed" in {
          ...
          running(application) {
            ...
            when(mockAuthConnector.authorise[RetrievalsType](any(), any())(any(), any()))
              .thenReturn(Future.successful(Some(testCredentials) ~ vatEnrolmentWithRegistration ~ Some(Individual) ~ ConfidenceLevel.L200 ~ None))
      ...
      "when the user has logged in as an Organisation Assistant with a VAT enrolment and strong credentials" - {
        ...
        "must be redirected to the Unsupported Credential Role page" in {
          ...
          running(application) {
            ...
            when(mockAuthConnector.authorise[RetrievalsType](any(), any())(any(), any()))
              .thenReturn(Future.successful(Some(testCredentials) ~ vatEnrolmentWithRegistration ~ Some(Organisation) ~ ConfidenceLevel.L50 ~ Some(Assistant)))
      ...
      "when the user has logged in as an Organisation Admin with strong credentials but no registration enrolment" - {
      ...
      "when the user has logged in as an Individual with a VAT enrolment and strong credentials, but confidence level less than 200" - {
        ...
        "must be redirected to uplift their confidence level" in {
          ...
          running(application) {
            ...
            when(mockAuthConnector.authorise[RetrievalsType](any(), any())(any(), any()))
              .thenReturn(Future.successful(Some(testCredentials) ~ vatEnrolmentWithRegistration ~ Some(Individual) ~ ConfidenceLevel.L50 ~ None))
    ...
    "when registration enrolments not enabled" - {
