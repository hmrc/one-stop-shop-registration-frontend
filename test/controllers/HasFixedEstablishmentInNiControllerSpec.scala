...
class HasFixedEstablishmentInNiControllerSpec extends SpecBase with MockitoSugar {
  ...
  private lazy val hasFixedEstablishmentInNiRoute = routes.HasFixedEstablishmentInNiController.onPageLoad(NormalMode).url
  ...
  "HasFixedEstablishmentInNi Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(HasFixedEstablishmentInNiPage, true).success.value
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        val result = route(application, request).value
        val expectedAnswers = emptyUserAnswers.set(HasFixedEstablishmentInNiPage, true).success.value

        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual HasFixedEstablishmentInNiPage.navigate(NormalMode, expectedAnswers).url
