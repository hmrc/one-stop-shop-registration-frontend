    ...
    "GET" - {
      ...
      "must return OK and the correct view when answers are complete" in {
        ...
        running(application) {
          ...
          val list = SummaryListViewModel(rows = getCYASummaryList(completeUserAnswers, dateService, registrationService, Seq.empty, CheckMode).futureValue)
        ...
        "the user is redirected when the incomplete prompt is shown" - {
          ...
          "to Previously Registered when previous registrations are not populated correctly" in {
            ...
            running(application) {
              ...
              status(result) `mustBe` SEE_OTHER
              redirectLocation(result).value `mustBe` controllers.previousRegistrations.routes.PreviouslyRegisteredController.onPageLoad(CheckMode).url
            }
          }
        }
      }
