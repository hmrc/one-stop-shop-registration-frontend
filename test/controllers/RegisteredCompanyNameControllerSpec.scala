...
import views.html.RegisteredCompanyNameView
...
class RegisteredCompanyNameControllerSpec extends SpecBase with MockitoSugar {
  ...
  "RegisteredCompanyName Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(RegisteredCompanyNamePage, "answer").success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, registeredCompanyNameRoute)
            .withFormUrlEncodedBody(("value", "answer"))

        val result = route(application, request).value
        val expectedAnswers = basicUserAnswers.set(RegisteredCompanyNamePage, "answer").success.value
