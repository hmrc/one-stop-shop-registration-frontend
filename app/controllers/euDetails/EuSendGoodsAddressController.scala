...
class EuSendGoodsAddressController @Inject()(
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = cc.authAndGetData() {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(EuSendGoodsAddressPage) match {
        case None => form
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = cc.authAndGetData().async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(EuSendGoodsAddressPage, value))
            _              <- cc.sessionRepository.set(updatedAnswers)
          } yield Redirect(EuSendGoodsAddressPage.navigate(mode, updatedAnswers))
      )
  }
}
