...
class EuSendGoodsTradingNameController @Inject()(
    ...
    implicit request =>
      getCountry(index) {
        country =>
          ...
          val preparedForm = request.userAnswers.get(EuSendGoodsTradingNamePage(index)) match {
            case None => form
      ...
      getCountry(index) {
        country =>
          ...
          form.bindFromRequest().fold(
            ...
            value =>
              for {
                updatedAnswers <- Future.fromTry(request.userAnswers.set(EuSendGoodsTradingNamePage(index), value))
                _ <- cc.sessionRepository.set(updatedAnswers)
              } yield Redirect(EuSendGoodsTradingNamePage(index).navigate(mode, updatedAnswers))
          )
      }
  }
