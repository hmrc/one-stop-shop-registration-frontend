...
object PreviousIossSchemeSummary  {
  ...
  def row(answers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] =
    answers.get(PreviousIossSchemePage).map {
      answer =>
        ...
        SummaryListRowViewModel(
          ...
          value   = value,
          actions = Seq(
            ActionItemViewModel("site.change", controllers.previousRegistrations.routes.PreviousIossSchemeController.onPageLoad(CheckMode).url)
